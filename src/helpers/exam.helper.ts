...
import { AnswerIndexEnum } from "src/constants/enums/answer.enums";
...
export class ExamHelper {
    ...
    private static seededShuffle(array: any[], seed: number) {
    ...
    static getQuestion(session: Session, questions: Question[]): { question: Question, choices: ExamChoices } {
    ...
    static gradeAnswers(session: Session, answers: Answer[]): { finalScore: number, remark: string } {
        let correctAnswers = 0;

        for (let answer of answers) {
            const shuffledChoices = this.seededShuffle(answer.question.choices, session.choiceSeed);
            if (answer.question.correctAnswer === shuffledChoices[AnswerIndexEnum[answer.selectedChoice]]) correctAnswers += 1;
        }

        const finalScore = Math.floor((correctAnswers / session.numberOfQuestions) * 100);
        let remark = this.performanceRemarks.EXCELLENT;

        if (finalScore <= 80) remark = this.performanceRemarks.GOOD;
        if (finalScore <= 60) remark = this.performanceRemarks.FAIR;
        if (finalScore <= 40) remark = this.performanceRemarks.POOR;
        if (finalScore <= 20) remark = this.performanceRemarks.ABYSMAL;
